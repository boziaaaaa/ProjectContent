...
class NPPProvider(DataProvider):
    ...
    def CreateBandsInfo(self):
        ...
        for wavelength in self.__BandWaveLenthList:
            ...
            elif 'SVM' in self.__filenamelist:
                if index<=11:
    ...
    def GetLongitude(self):
        ...
        if 'GITCO_' in HdfFileHandleList_str:
            fileHandle = self.__HdfFileHandleList['Longitude']
            hdfgroup = fileHandle['All_Data/VIIRS-IMG-GEO-TC_All']
            Longitude = hdfgroup['Longitude']
            Longitude = Longitude[:, :].astype(N.float32)
            #return Longitude
            #return self.GetDataSet(self.__HdfFileHandleList['Longitude'], 'All_Data','VIIRS-IMG-GEO-TC_All/Longitude')
        elif 'GMTCO_' in HdfFileHandleList_str:
            Longitude = self.GetDataSet(self.__HdfFileHandleList['Longitude'], 'All_Data','VIIRS-MOD-GEO-TC_All/Longitude')
        else:
            Longitude =  self.GetDataSet(self.__HdfFileHandleList['Longitude'], 'All_Data', 'VIIRS-DNB-GEO_All/Longitude')
        Longitude = self.ResizeData(Longitude)
        print("LLLLLLLLLLLLLLLLLLLLooooooooooooooooooonnnnnnnnnnn")
        size = Longitude.shape
        print(size)
        return Longitude
    ...
    def GetLatitude(self):
        ...
        if 'GITCO_' in HdfFileHandleList_str:
            ...
            hdfgroup = fileHandle['All_Data/VIIRS-IMG-GEO-TC_All']
            Latitude = hdfgroup['Latitude']
            Latitude = Latitude[:, :].astype(N.float32)
            #return Latitude
            #return self.GetDataSet(self.__HdfFileHandleList['Latitude'], 'All_Data', 'VIIRS-IMG-GEO-TC_All/Latitude')
        elif 'GMTCO_' in HdfFileHandleList_str:
            Latitude = self.GetDataSet(self.__HdfFileHandleList['Latitude'], 'All_Data','VIIRS-MOD-GEO-TC_All/Latitude')
        else:
            Latitude = self.GetDataSet(self.__HdfFileHandleList['Latitude'], 'All_Data', 'VIIRS-DNB-GEO_All/Latitude')
        Latitude = self.ResizeData(Latitude)
        print("LLLLLLLLLLLLLLLLlaaaaaaaaaaaaaaaattttttttt")
    ...
    def GetOBSData(self, band):
        ...
        if 'SVI' in self.__filenamelist:
                ...
                print(each_file)
                if index <=3:
                    hdfgroup = each_file['All_Data/'+groupname]
                    ret = hdfgroup['Reflectance']
                    ret = ret[:, :].astype('f4')
                    ret = N.array(ret)
                    ReflectanceFactors = hdfgroup['ReflectanceFactors']
                    Slope = ReflectanceFactors[0].astype('f4')
                    Offset = ReflectanceFactors[1].astype('f4')
                    print("RRRRRRRRRRRR")
                    print(Slope)
                    print(Offset)
                    #ret.dtype = 'float32' #DO NOT use .dtype='f4' .because this can change the data value(random? I do not know)
                    print(ret[ret<50000])
                    ret = ret * Slope + Offset
                    ret = ret *1000
                    t = 50000* Slope + Offset    
                    t= t*1000
                    print(ret[ret<t])                
                    #ret = self.GetDataSet(each_file, 'All_Data',groupname + '/Reflectance')[:, :].astype(N.int32)
                else:
                    hdfgroup = each_file['All_Data/'+groupname]
                    ret = hdfgroup['Radiance']
                    #ret = ret[:, :].astype(N.int32)
                    ret = ret[:, :].astype('f4')
                    RadianceFactors = hdfgroup['RadianceFactors']
                    Slope = RadianceFactors[0]
                    Offset = RadianceFactors[1]	
                    ret = ret * Slope + Offset 
                    ret = ret * 1000
                    print(ret[ret<35000])                
        ...
        ret = self.ResizeData(ret)

        print("RRRRRRRRRRRRRREEEEEEEEEEEEEEEETTTTTTTTTTT")
        print(band)
        print(ret.shape)
        return ret
    #
    ...
    def GetAuxiliaryData(self,dataname):
        ...
        if dataname == 'Latitude':
            data = self.GetLatitude()
            return data
        elif dataname == 'Longitude':
            data = self.GetLongitude()
            return data
        if 'GITCO_' in HdfFileHandleList_str:
